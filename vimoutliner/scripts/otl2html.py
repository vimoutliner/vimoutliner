...
def colonStrip(line):
        if (line[0] == ":"): return line[1:].lstrip()
        else: return line
...
def handleBodyText(linein,lineLevel):
  ...
  if (styleSheet != ""):
    ...
    inBodyText = 1
  print(">" + colonStrip(linein.lstrip().rstrip()),)
...
def handlePreformattedText(linein,lineLevel):
  ...
  if (styleSheet != ""):
    ...
    inBodyText = 2
  print(">" + semicolonStrip(linein.lstrip().rstrip()),)
...
def includeFile(line,lineLevel):
  filename = sub('!(\S+?)!','\\1',line.rstrip().lstrip())
...
def execProgram(line):
  program = sub('.*!!!(.*)!!!.*','\\1',line.rstrip().lstrip())
  child = popen(program)
  out = child.read()
  err = child.close()
  out = sub('!!!(.*)!!!',out,line)
...
def beautifyLine(line):
  ...
  while (line != out):
          ...
          line = out
        # out = out.replace(out,'---','<strike>',1)
          if (line.lstrip()[0] != ";"): out = sub('\-\-\-(.*?)\-\-\-','<strike>\\1</strike>',out)
          out = linkOrImage(out)
  ...
  if ((hideComments == 0) or (lineLevel != (linein.find("[")+1))):
      ...
      if (slides == 0):
          ...
          (lineLevel == linein.find(":") +1 ): 
                  ...
                  elif (colonStrip(linein.lstrip().rstrip()) == ""):
                          print("</p>")
                          handleBodyText(linein,lineLevel)
                  else: print(colonStrip(linein.lstrip().rstrip()),)
          elif (lineLevel == linein.find(";") +1 ): 
                  if (inBodyText != 2): handlePreformattedText(linein,lineLevel)
                  elif (semicolonStrip(linein.lstrip().rstrip()) == ""):
                          print("</pre>")
                          handlePreformattedText(linein,lineLevel)
                  else: print(semicolonStrip(linein.lstrip().rstrip()),)
          elif (lineLevel == linein.find("|") +1 ): 
                  if (inBodyText != 3): handleTable(linein,lineLevel)
                  elif (pipeStrip(linein.lstrip().rstrip()) == ""):
                          print("</table>")
            ...
            if (silentdiv == 0):
                    ...
                    if (styleSheet != ""):
                      if (lineLevel == linein.find("- ") +1 ): 
                        ...
                        print(">" +
                                dashStrip(linein.lstrip()).lstrip().rstrip(),)
                      elif (lineLevel == linein.find("+ ") +1 ): 
                        print(" class=\"LN" + str(lineLevel) + "\"",)
                        print(">" +
                                plusStrip(linein.lstrip()).lstrip().rstrip(),)
                      else:
                        print(" class=\"L" + str(lineLevel) + "\"",)
                        print(">" + linein.lstrip().rstrip(),)
            else: silentdiv = 0
      else:
          if (lineLevel == 1):
            if (linein[0] == " "):
              if (inBodyText == 0):
                handleBodyText(linein,lineLevel)
              else: print(linein.lstrip().rstrip(),)
            else:
              print("<address>")
              print(linein.lstrip().rstrip(),)
              print("</address>\n")
...
def printFirstLine(linein):
  ...
  print("<h1>" + stripTitleText(linein.lstrip()) +"</h1>")
  print("</div>")
...
def main():
  ...
  if (slides == 0):
    firstLine = beautifyLine(file.readline().rstrip().lstrip())
    printHeader(firstLine)
    if (showTitle == 1):
      printFirstLine(firstLine)
      linein = beautifyLine(file.readline().lstrip().rstrip())
    else:
      linein = firstLine
    while linein != "":
      processLine(linein)
    ...
    linein = file.readline().lstrip().rstrip()
    while linein != "":
      outline.append("\t" + linein)
